  ...
  with dmModule do begin
    ...
    if cboFornecedores.ItemIndex <> -1 Then begin
     ...
     IDProdEstoque := ibEntrProdEstoque.FieldByName('IDPROD').AsInteger;
      ...
      QtdeMinima  := StrToFloat(StringReplace(FloatToStr(ibEntrProdEstoque.FieldByName('ESTOQUEMIN').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
   ...
   //  if (QtdeEstoque >-1) Then begin
      ...
      QtdeCompra := StringReplace(FloatToStr(QtdeParaComprar),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      QtdeCompra := StringReplace(QtdeCompra,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
