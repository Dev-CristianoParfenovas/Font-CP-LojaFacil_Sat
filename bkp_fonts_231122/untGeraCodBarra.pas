...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, IBQuery,
  Buttons;
...
type
  TfrmCodBarrasAdicional = class(TForm)
    ...
    Image1: TImage;
    ibeaIncluiCodBarras: TSpeedButton;
...
begin
  ...
  with dmModule do begin
    ...
    if lbl_edtCodBarra.Text <> '' Then begin
      ...
      Application.MessageBox('ATENÇÃO este código já existe em outro produto,digite um novo número','Código existente',+MB_OK+MB_DEFBUTTON1+MB_ICONWARNING);
      lbl_edtCodBarra.SetFocus;
        ...
        if ibProdutos.FieldByName('CODBARRASAD').AsString = '' Then begin
          ...
          ConfirmaMSG := Application.MessageBox( Pchar('Código adicional gerado para o Produto: "' + lbl_edtNomeProduto.Text
           +'"'),'Notificação:Inclusão de Código de Barras Adicional', MB_YESNO + MB_DEFBUTTON1 + MB_ICONQUESTION);
          ...
          ConfirmaMSG := Application.MessageBox( Pchar('O Produto: "' + lbl_edtNomeProduto.Text
           +'" já contém Código de Barras Adicional, deseja alterar? Se Sim Clique em ok'),'Notificação:Alterarção de Código de Barras Adicional', MB_YESNO + MB_DEFBUTTON1 + MB_ICONQUESTION);
