...
uses
  ...
  ComCtrls, ExtCtrls, IBQuery, classComboBox,
  IniFiles, jpeg, DB,axCtrls, Menus, untClasse.Cadastro, untClasse.DataEntrEstoque;
...
type
  TfrmProd_MPrima = class(TForm)
    ...
    cboFornecedores2: TComboBox;
    cboFornecedores3: TComboBox;
    cboFornecedores4: TComboBox;
    ...
    cboFornecedores5: TComboBox;
    cboFornecedores6: TComboBox;
    cboFornecedores7: TComboBox;
    cboFornecedores8: TComboBox;
    ...
    Label35: TLabel;
    btIncluirEquivalente: TSpeedButton;
    btExcluirEquivalente: TSpeedButton;
    ...
    cboFornecedoresProdutos: TComboBox;
    lbl_edtVlrCompra: TLabeledEdit;
    btnRProdEstoque: TBitBtn;
    pcFornecedores: TPageControl;
    tbs1Forn: TTabSheet;
    dbgFornecedorProd: TDBGrid;
    tbsConfig: TTabSheet;
    lbl_edtCodFabr: TLabeledEdit;
    lbl_edtComissaoProd: TLabeledEdit;
    lbl_edtLocalizacaoProd: TLabeledEdit;
    lbl_edtBuscaCodFabr: TLabeledEdit;
    lbl_edtComissaoC: TLabeledEdit;
    tbsHistEstoq: TTabSheet;
    dbgHistEstoq: TDBGrid;
    Label5: TLabel;
    ...
    procedure lbl_edtCodFabrEnter(Sender: TObject);
    procedure lbl_edtCodFabrExit(Sender: TObject);
    procedure lbl_edtCodFabrKeyPress(Sender: TObject; var Key: Char);
    procedure lbl_edtComissaoProdKeyPress(Sender: TObject; var Key: Char);
    procedure lbl_edtComissaoProdEnter(Sender: TObject);
    procedure lbl_edtComissaoProdExit(Sender: TObject);
    procedure lbl_edtBuscaCodFabrChange(Sender: TObject);
    procedure lbl_edtBuscaCodFabrEnter(Sender: TObject);
    procedure lbl_edtBuscaCodFabrExit(Sender: TObject);
    procedure lbl_edtQtdePacoteFardoExit(Sender: TObject);
    procedure lbl_edtComissaoCEnter(Sender: TObject);
    procedure lbl_edtComissaoCExit(Sender: TObject);
    procedure lbl_edtComissaoCKeyPress(Sender: TObject; var Key: Char);
    procedure ckAcrescentarEstoqClick(Sender: TObject);
    procedure ckDiminuirEstoqueClick(Sender: TObject);
...
//  TotalML : String;
  ...
  MontaColunasMP, MontaColunaEstoqueProd : Boolean;
  IDProdutoMP : Integer;
...
uses  uBuscaProdutos, funcPosto, untdmModule, untGeraCodBarra,
  ...
  untPrincipal, Types, untRProdFornecedor, untGradeProdutos,
  Firebase.Interfaces,
  Firebase.Auth,
  Firebase.Database,
  System.JSON,
...
procedure TfrmProd_MPrima.TrimAppMemorySize;
var
MainHandle : THandle;
begin
try
...
//  lbl_edtUncompra.Enabled := opcao;
  ...
  lbl_edtComissaoProd.Enabled := opcao;
  lbl_edtComissaoC.Enabled := opcao;
...
//  lbl_edtMls.Enabled := opcao;
 ...
 // cboModalidade.Enabled := opcao;
  ...
  lbl_edtLocalizacaoProd.Enabled := opcao;
...
begin
  with dmModule do begin
    ...
    IDClassFornProd := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    cboCSOSN.Clear; //LIMPA O COMBOBOX  +'..'+ibCSOSN.FieldByName('DESCRICAOCSOSN').AsString,ClassCSOSN
...
//  lbl_edtMls.Text := '0';
  ...
  lbl_edtComissaoProd.Clear;
  lbl_edtComissaoC.Clear;
  lbl_edtLocalizacaoProd.Clear;
  lbl_edtBuscaCodFabr.Clear;
...
var
  ...
  DataLote : TDate;
  VctoLote : String;
  intNacProd,idCST,idCSOSN,idCFOP : Integer;
...
//    (ibProdutos.FieldByName('IDPROD') as TIntegerField).DisplayFormat := '0000000000';
    ...
    ibConfig.Open;
     ...
     // cboCFOP.ClearSelection;
      for i:=0 to cboCFOP.Items.Count -1 do begin
        if TClasseCombo( cboCFOP.Items.Objects[i]).ID = ibProdutos.FieldByName('IDCFOP').AsInteger then begin
          cboCFOP.ItemIndex:= i;
          Break;
        end else begin
          cboCFOP.ClearSelection;
      ...
      for i:=0 to cboCSOSN.Items.Count -1 do begin
        if TClasseCombo( cboCSOSN.Items.Objects[i]).ID = ibProdutos.FieldByName('IDCSOSN').AsInteger then begin
    ...
    for i:=0 to cboSubgrupo.Items.Count -1 do begin
      ...
      //cboCST.ClearSelection;
      for i:=0 to cboCST.Items.Count -1 do begin
        if TClasseCombo( cboCST.Items.Objects[i]).ID = ibProdutos.FieldByName('IDCST').AsInteger then begin
          cboCST.ItemIndex:= i;
          Break;
        end else begin
          cboCST.ClearSelection;
   ...
   // if ibProdutos.FieldByName('IDUNIDVENDA').AsInteger > 0 Then begin
    ...
    lbl_edtComissaoProd.Text        := FormatFloat(CasasDecimais('Produtos'),ibProdutos.FieldByName('COMISSAOPROD').AsFloat);
    lbl_edtComissaoC.Text        := FormatFloat(CasasDecimais('Produtos'),ibProdutos.FieldByName('COMISSAOPRECOC').AsFloat);
    lbl_edtLocalizacaoProd.Text     := ibProdutos.FieldByName('LOCALIZACAOPROD').AsString;
    ...
    lbl_edtCodFabr.Text             := ibProdutos.FieldByName('CODIGOFABRICANTE').AsString;
...
//    lblEstoqueNomeProd.Caption         := ibProdutos.FieldByName('NOME').AsString;
    ...
    lbl_edtEstoqgeral.Text  := FormatFloat(CasasDecimais('Combustiveis'),ibEntrProdEstoque.FieldByName('QUANTIDADE').AsFloat);
    ...
    lbl_edtQtdeEntrada.Text := FormatFloat(CasasDecimais('Combustiveis'),ibEntrProdEstoque.FieldByName('QTDEENTRADA').AsFloat);//FloatToStr(ibEntrProdEstoque.FieldByName('QTDEENTRADA').AsFloat);
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn2 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn3 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn4 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn5 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn6 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn7 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
begin
  with dmModule do begin
    ...
    ibClassificacao.SQL.Add('SELECT IDCLASS FROM TBLCLASSIFICACAO WHERE DESCRICAO=''Fornecedores''');
    ibClassificacao.Open;
    IDClassForn8 := ibClassificacao.FieldByName('IDCLASS').AsInteger; // ATRIBUI A IDENTIFICAÇÃO DO FORNECEDOR NA VARIAVEL.
...
TotalGeralQtde, TotalMinimo, TotalVlrEstoque, VlrTotalCusto, StringAltForn : String;
...
intIDProdEstoque, strQtdeEstoqueProd : TDataEntrEstoque;
ClassAltForn : TClasseCombo;
begin
    ...
    if (lbl_edtNomeProduto.Text <> '')and(lbl_edtCustoFormula.Text = '0,00')and(lbl_edtQtdeFormula.Text = '0,00') Then begin
      ...
      if ckAcrescentarEstoq.Checked Then begin
        ...
        if (lbl_edtQtdeEstoque.Text <> '') or (lbl_edtVendaa.Text <> '') Then begin
          ...
          QtdeGeral := StrToFloat(StringReplace(FloatToStr(ibEntrProdEstoque.FieldByName('QUANTIDADE').AsFloat),FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
          ...
          TotalQtdeGeralAlt := StrToFloat(StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

          end;

          QtdeGeral := QtdeGeral + TotalQtdeGeralAlt;

          TotalGeralQtde := StringReplace(FloatToStr(QtdeGeral),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
          TotalGeralQtde := StringReplace(TotalGeralQtde,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);

        /////////////////////////////////////////////////////////////////////////

          //rotina para calculo do vlr total do estoque
          NovoValor   := StrToFloat(StringReplace(lbl_edtVendaa.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
          VlrGeral    := NovoValor * QtdeGeral;

          TotalVlrEstoque := StringReplace(FloatToStr(VlrGeral),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
          TotalVlrEstoque := StringReplace(TotalVlrEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
        ...
        //////////////////////////////////////////////////////////////////////////
          ...
          if (lbl_edtCustoFormula.Text > '0,00')and(lbl_edtQtdeFormula.Text > '0,00')and(lbl_edtResultado.Text > '0,00') Then begin
            ...
            VlrCustoFormula := StrToFloat(StringReplace(lbl_edtResultado.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
            QtdeFormula     := StrToFloat(StringReplace(lbl_edtQtdeFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
            RCustoFormula   := VlrCustoFormula * QtdeGeral;

            VlrTotalCusto := StringReplace(FloatToStr(RCustoFormula),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            VlrTotalCusto := StringReplace(VlrTotalCusto,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);

           /// VlrCustoNovoProduto := StrToFloat(StringReplace(lbl_Resultado.Text,ThousandSeparator,'',[rfReplaceAll]));
            VlrCustoNovo  := StringReplace(lbl_edtResultado.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            VlrCustoNovo  := StringReplace(VlrCustoNovo,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            ...
            TotalCusto := StrToFloat(StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
            VlrCusto   := TotalCusto * QtdeGeral;

            VlrTotalCusto := StringReplace(FloatToStr(VlrCusto),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            VlrTotalCusto := StringReplace(VlrTotalCusto,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
       ...
       //  if lbl_edtEstoqueM.Text <> '' Then begin
            ...
            TotalMinimo := StringReplace(lbl_edtEstoqueM.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            TotalMinimo := StringReplace(TotalMinimo,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            if TotalMinimo = '' Then
            TotalMinimo := '0';

            QtdeEntrada := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            QtdeEntrada := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            if QtdeEntrada = '' Then
            QtdeEntrada := '0';
       ...
       //   end;//if
         ...
         /////////////////////////////////////////////////////////////////////////
          ...
          if ibEntrProdEstoque.RecordCount > 0 Then begin
              ...
              //  Erro := True;
                StringAltForn    := ' FORNECEDOR= ''0'',';

              end else begin
                ClassAltForn      := TClasseCombo(cboFornecedores.Items.Objects[cboFornecedores.ItemIndex] );                StringAltForn     := ' FORNECEDOR= ''' + InttoStr( ClassAltForn.ID ) + ''',';
            ...
            ValorTotalE := StringReplace(lbl_edtVlrEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            ValorTotalE := StringReplace(ValorTotalE,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            if ValorTotalE = '' Then
            ValorTotalE := '0';

            QtdeProdEstoqueE := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            QtdeProdEstoqueE := StringReplace(QtdeProdEstoqueE,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            ...
            VlrCustoProd := StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            VlrCustoProd := StringReplace(VlrCustoProd,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
                ClassAltForn      := TClasseCombo(cboFornecedores.Items.Objects[cboFornecedores.ItemIndex] );                StringAltForn     := ' ''' + InttoStr( ClassAltForn.ID ) + ''',';
            ...
            + '(QUANTIDADE,ESTOQUEMIN,IDPROD,VLRTOTAL,IDLOCAL,FORNECEDOR,'
        ...
        lbl_edtVendaFormula.Text := '0,00';
                ...
                intIDProdEstoque := TDataEntrEstoque.Create();
                strQtdeEstoqueProd := TDataEntrEstoque.Create();

              try

                intIDProdEstoque.IDProduto := IDProd;

                strQtdeEstoqueProd.QtdeEstoque := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
                strQtdeEstoqueProd.QtdeEstoque := StringReplace(strQtdeEstoqueProd.QtdeEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
                if strQtdeEstoqueProd.QtdeEstoque = '' Then
                  strQtdeEstoqueProd.QtdeEstoque := '0';

                ibDataEntrEstoque.Close;
                ibDataEntrEstoque.SQL.Clear;
                ibDataEntrEstoque.SQL.Add('INSERT INTO TBLDATAENTRESTOQUE '
                + '(QTDE,DATA,IDPROD) values '
                + '(''' + strQtdeEstoqueProd.QtdeEstoque + ''','
                + ' ''' + FormatDateTime('mm-dd-yyyy',now) + ''','
                + ' ''' + IntToStr(intIDProdEstoque.IDProduto) + ''')');
                ibDataEntrEstoque.ExecSQL;

              finally

                FreeAndnil(intIDProdEstoque);
                FreeAndnil(strQtdeEstoqueProd);
      ...
      if ckDiminuirEstoque.Checked Then begin
        ...
        QtdeEstoque := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        QtdeEstoque := StringReplace(QtdeEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
        if QtdeEstoque = '' Then
          QtdeEstoque := '0';

        EstoqueMin := StringReplace(lbl_edtEstoqueM.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        EstoqueMin := StringReplace(EstoqueMin,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
        if EstoqueMin = '' Then
          EstoqueMin := '0';

        VlrEstoque := StringReplace(lbl_edtVlrEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        VlrEstoque := StringReplace(VlrEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
        if VlrEstoque = '' Then
          VlrEstoque := '0';
...
//////////////////////////////////////////////////////////////////////////
      ...
      // rotina para somar o custo do produto no estoque
       TotalCusto := StrToFloat(StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
       QtdeGeral  := StrToFloat(StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
...
//****************************************************************************//
    ...
    if (lbl_edtNomeProduto.Text <> '')and(lbl_edtCustoFormula.Text > '0,00')and(lbl_edtQtdeFormula.Text > '0,00') Then begin
        ...
        if (lbl_edtQtdeFormula.Text <> '') or (lbl_edtVendaa.Text <> '') Then begin
          ...
          TotalQtdeGeralAlt := StrToFloat(StringReplace(lbl_edtQtdeFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
    ...
    {  if ckAlterarPrateleira.Checked Then begin
      ...
      if (ckAcrescPrateleira.Checked)and(lbl_edtEstoqgeral.Text > '0') Then begin
        ...
        if (VerifPrateleira > VerifEstoque) Then begin
          ...
          Application.MessageBox('Atenção a qtde acrescentada na prateleira está maior que a qtde do estoque?? ','Erro',+MB_OK+MB_DEFBUTTON1+MB_ICONERROR);
        ...
        if (VerifPrateleira < VerifEstoque)or(VerifPrateleira = VerifEstoque) Then begin
          ...
          //rotina para somar a qtde de entrada no estoque
            ...
            PrateleiraAtual     := StrToFloat(StringReplace(lbl_edtQtdePrat.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

            if lbl_edtEstoqgeral.Text > '0' Then begin

              QtdeEstoque := StrToFloat(StringReplace(lbl_edtEstoqgeral.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
            ...
            if lbl_edtInfoQP.Text = '' Then begin
              ...
              TotalQtdeGeralPrateleira := StrToFloat(StringReplace(lbl_edtInfoQP.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

            end;

            QtdeGeralPrateleira := StrToFloat(StringReplace(lbl_edtQtdePrat.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));

            QtdeGeralPrateleira := QtdeGeralPrateleira + TotalQtdeGeralPrateleira;

            TotalPratAlterado := StringReplace(FloatToStr(QtdeGeralPrateleira),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            TotalPratAlterado := StringReplace(TotalPratAlterado,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
        ...
        //////////////////////////////////////////////////////////////////////////
          ...
          if ibEstoquePrateleira.RecordCount > 0 Then begin
            ...
            TotalMinimoPratAlt := StringReplace(lbl_edtMinimoPrat.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            TotalMinimoPratAlt := StringReplace(TotalMinimoPratAlt,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            if TotalMinimoPratAlt = '' Then
            TotalMinimoPratAlt := '0';

            ValorProduto := StrToFloat(StringReplace(FloatToStr(ibEstoquePrateleira.FieldByName('VALORVENDAA').AsFloat),FormatSettings.ThousandSeparator,'',[rfreplaceAll]));
            ValordoEstoque := QtdeEstoque * ValorProduto;
            ValorTotalEstoque := StringReplace(FloatToStr(ValordoEstoque),FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            ValorTotalEstoque := StringReplace(ValorTotalEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            ...
            TotalPratAlterado := StringReplace(lbl_edtQtdePrat.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            TotalPratAlterado := StringReplace(TotalPratAlterado,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            if TotalPratAlterado = '' Then
            TotalPratAlterado := '0';
...
var
  ...
  NacProd : Integer;
  strICMS,strCOFINS,strIPI,strPIS,strComissaoProd,strComissaoPrecoC :String;
  strNomeProd : TProdutos;
...
// Erro := False;
  ...
  //instanciaando a classe TProdutos
  strNomeProd := TProdutos.Create();
...
////////////////////////////////////////////////////////////////////////////////
    ...
    If (LocalSelecionado = -1) Then begin
      ...
      ValorCusto := StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      ...
      ValorVendaa := StringReplace(lbl_edtVendaa.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      ValorVendaa := StringReplace(ValorVendaa,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if ValorVendaa = '' Then
        ValorVendaa := '0';

      ValorVendab := StringReplace(lbl_edtVendab.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      ValorVendab := StringReplace(ValorVendab,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if ValorVendab = '' Then
        ValorVendab := '0';

      QtdeB := StringReplace(lbl_edtQtdeB.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      QtdeB := StringReplace(QtdeB,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if QtdeB = '' Then
         QtdeB := '0';

      ValorVendac := StringReplace(lbl_edtVendac.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      ValorVendac := StringReplace(ValorVendac,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if ValorVendac = '' Then
        ValorVendac := '0';

      QtdeC := StringReplace(lbl_edtQtdeC.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      QtdeC := StringReplace(QtdeC,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if QtdeC = '' Then
         QtdeC := '0';

      ValorVendad := StringReplace(lbl_edtVendad.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      ValorVendad := StringReplace(ValorVendad,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if ValorVendad = '' Then
        ValorVendad := '0';
      ...
      VlrPacoteFardo := StringReplace(lbl_edtVlrPacoteFardo.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      VlrPacoteFardo := StringReplace(VlrPacoteFardo,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if VlrPacoteFardo  = '' Then
         VlrPacoteFardo := '0';

      QtdePacoteFardo := StringReplace(lbl_edtQtdePacoteFardo.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      QtdePacoteFardo := StringReplace(QtdePacoteFardo,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if QtdePacoteFardo  = '' Then
         QtdePacoteFardo := '0';

      strIPI := StringReplace(lbl_edtIPI.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      strIPI := StringReplace(strIPI,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if strIPI = '' Then
         strIPI := '0';

      strPIS := StringReplace(lbl_edtPIS.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      strPIS := StringReplace(strPIS,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if strPIS = '' Then
         strPIS := '0';

      strICMS := StringReplace(lbl_edtICMS.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      strICMS := StringReplace(strICMS,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if strICMS = '' Then
         strICMS := '0';

      strCOFINS := StringReplace(lbl_edtCOFINS.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      strCOFINS := StringReplace(strCOFINS,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if strCOFINS = '' Then
         strCOFINS := '0';

      strComissaoProd := StringReplace(lbl_edtComissaoProd.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      strComissaoProd := StringReplace(strComissaoProd,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if strComissaoProd = '' Then
         strComissaoProd := '0';

      strComissaoPrecoC := StringReplace(lbl_edtComissaoC.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      strComissaoPrecoC := StringReplace(strComissaoPrecoC,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
      if strComissaoPrecoC = '' Then
         strComissaoPrecoC := '0';
      ...
      if ckProdTrib.Checked Then begin
        ...
        AltProdTrib := 'NAO';
          ...
          try
            ...
            strNomeProd.Nome := lbl_edtNomeProduto.Text;
...
////////////////////////////////////////////////////////////////////////////////
      ...
      ////////////////////////////////////////////////////////////////////////////////
              ...
              //GRAVA AS ALTERACOES NA TABELA DE PRODUTOS.
                ...
                + ' CODIGOFABRICANTE = ''' + lbl_edtCodFabr.Text + ''','
                + ' LUCROVENDA = ''' + lbl_edtLucroVenda.Text + ''','
                + ' LOCALIZACAOPROD = ''' + lbl_edtLocalizacaoProd.Text + ''','
                + StringGrupo
                + StringSubGrupo
                + ' REF = ''' + lbl_edtRef.Text + ''','
                + StringLocalEstoque
                ...
                + ' ICMS = '''+ (strICMS)+''','
                + ' COFINS = '''+ (strCOFINS)+''','
                + ' COMISSAOPROD = '''+ (strComissaoProd)+''','
                + ' COMISSAOPRECOC = '''+ (strComissaoPrecoC)+''','
...
var
  ...
  intTipoOleo, intNacProd, intGravarNuvem,intNuvem : Integer;

  //atributos

  idClassCST,idClassST,idClassCSOSN,idClassCFOP : TClasseCombo;
  strCST,strST,strCSOSN,strCFOP,strNCMSH : String;
  strICMS,strCOFINS,strIPI,strPIS,strComissaoProd,strComPrecoC :String;
  intIdProduto : Integer;
  strNomeProd : TProdutos;

  //variaveis da classe dataentrestoque
  intIDProdEstoque, strQtdeEntrEstoque : TDataEntrEstoque;
...
////////////////////////////////////////////////////////////////////////////////
          ...
          If (LocalSelecionado = -1) Then begin
            ...
            QtdeUnidVenda := StringReplace(lbl_edtQtde.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            QtdeUnidVenda := StringReplace(QtdeUnidVenda,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            ...
            strComPrecoC := StringReplace(lbl_edtComissaoC.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
            strComPrecoC := StringReplace(strComPrecoC,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
            if strComPrecoC = '' Then
               strComPrecoC := '0';
...
////////////////////////////////////////////////////////////////////////////////
                ...
                if intNuvem = 1 then begin

                  intGravarNuvem := Application.MessageBox('Confirma o registro na nuvem','Notificação:Inclusão de registro',+MB_YESNO+MB_ICONINFORMATION);
                  ...
                  if intGravarNuvem = 6 then begin
                     ...
                     //     {"6": {"cod": 6, "descricao": "Fone ouvido"} }
                      ...
                      Writer.WritePropertyName(lbl_edtCodigo.Text);

                      Writer.WriteStartObject;
                      Writer.WritePropertyName('idprod');
                      Writer.WriteValue(lbl_edtCodigo.Text);
                      Writer.WritePropertyName('nome');
                      Writer.WriteValue(lbl_edtNomeProduto.Text);
                      ...
                      try
                       // AResponse := ADatabase.Post([strNode + '.json'], JSONReq);
                        //AResponse := ADatabase.Put([edtNode.Text + '.json'], JSONReq);
                        AResponse := ADatabase.Patch([strNode + '.json'], JSONReq);
...
////////////////////////////////////////////////////////////////////////////////
              ...
              if ibConfig.FieldByName('TRESCASASDECIMAIS').AsString = 'Sim' Then begin
                ...
                (ibProdutos.FieldByName('VALORCUSTO') as TFloatField).DisplayFormat  := CasasDecimais('Combustiveis'); //CRIA UMA MASCARA PARA O CAMPO
                (ibProdutos.FieldByName('VALORVENDAA') as TFloatField).DisplayFormat := CasasDecimais('Combustiveis');
                (ibProdutos.FieldByName('VALORVENDAB') as TFloatField).DisplayFormat := CasasDecimais('Combustiveis');
                (ibProdutos.FieldByName('VALORVENDAC') as TFloatField).DisplayFormat := CasasDecimais('Combustiveis');
              ...
              if ibConfig.FieldByName('TRESCASASDECIMAIS').AsString = 'Nao' Then begin
...
////////////////////////////////////////////////////////////////////////////////
           ...
           if (lbl_edtQtdeEstoque.Text <> '')Then begin
              ...
              ValorTotal := StringReplace(lbl_edtVlrEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
              ValorTotal := StringReplace(ValorTotal,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
              if ValorTotal = '' Then
                ValorTotal := '0';

              QtdeProdEstoque := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
              QtdeProdEstoque := StringReplace(QtdeProdEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
              ...
              QtdePrateleira := StringReplace(lbl_edtQtdePrat.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
              QtdePrateleira := StringReplace(QtdePrateleira,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
              if QtdePrateleira = '' Then
                QtdePrateleira := '0';

              MinimoPrateleira := StringReplace(lbl_edtMinimoPrat.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
              MinimoPrateleira := StringReplace(MinimoPrateleira,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
             ...
             If cboFornecedores.ItemIndex = -1 Then begin
              ...
              ibEntrProdEstoque.SQL.Clear;
              ibEntrProdEstoque.SQL.Add('INSERT INTO TBLENTRADAESTOQUEPROD '
              + '(QUANTIDADE,ESTOQUEMIN,IDPROD,IDLOCAL,QTDEPRATELEIRA,MINIMOPRATELEIRA,'
            ...
            ///  + StringLocalEstoque  IDLOCAL,
              ...
              intIDProdEstoque := TDataEntrEstoque.Create();
              strQtdeEntrEstoque := TDataEntrEstoque.Create();

              try

                intIDProdEstoque.IDProduto := IDProdEstoque;

                strQtdeEntrEstoque.QtdeEstoque := StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
                strQtdeEntrEstoque.QtdeEstoque := StringReplace(strQtdeEntrEstoque.QtdeEstoque,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
                if strQtdeEntrEstoque.QtdeEstoque = '' Then
                  strQtdeEntrEstoque.QtdeEstoque := '0';

                ibDataEntrEstoque.Close;
                ibDataEntrEstoque.SQL.Clear;
                ibDataEntrEstoque.SQL.Add('INSERT INTO TBLDATAENTRESTOQUE '
                + '(QTDE,DATA,IDPROD) values '
                + '(''' + strQtdeEntrEstoque.QtdeEstoque + ''','
                + ' ''' + FormatDateTime('mm-dd-yyyy',now) + ''','
                + ' ''' + IntToStr(intIDProdEstoque.IDProduto) + ''')');
                ibDataEntrEstoque.ExecSQL;
 ...
 ////////////////////////////////////////////////////////////////////////////////
       ...
       //   end;{if tambem do tipo de produto}
        ...
        //  DesabilitaCampos(False);
          ...
          ibTempProdNome.SQL.Clear;                                  //  ORDER BY IDPROD DESC
          ibTempProdNome.SQL.Add('SELECT (IDPROD) FROM CADPRODUTOS ORDER BY IDPROD DESC');
          ibTempProdNome.Open;

          lbl_edtCodigo.Text := IntToStr(ibTempProdNome.FieldByName('IDPROD').AsInteger + 1);


        //  intIdProduto := StrToInt(lbl_edtCodigo.Text);
        //  lbl_edtCodigo.Text := intToStr(intIdProduto + 1);
...
begin
 ...
 //dbgProdutos.options := [dgEditing,dgTitles,dgIndicator,dgColLines,dgRowLines,dgConfirmDelete];
 ...
 //DEFINE COM QUE A PAGEINDEX FIQUE COMO 0
  ...
  with dmModule do begin
        ...
        ibConfig.Close;
        ibConfig.SQL.Clear;
        ibConfig.SQL.Add('SELECT TRESCASASDECIMAIS FROM TBLCONFIGURACOES');
        ibConfig.Open;
    ...
    with dbgProdutos do begin
      ...
      if MontaColunas Then begin
       ...
       // Parametros que exibe o valor de venda (b)
        ...
        // Parametros que exibe o valor de venda (a)
        Columns.Items[7].Title.Caption := 'Cód.Fabr';
        Columns.Items[7].FieldName     := 'CODIGOFABRICANTE';
        Columns.Items[7].Width         := 77;
        Columns.Items[7].Alignment     := taLeftJustify;

        // Parametros que exibe o valor de venda (c)
        Columns.Items[8].Title.Caption := 'Data Alteração';
        Columns.Items[8].FieldName     := 'DATAALTERACAO';
        Columns.Items[8].Width         := 77;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    with dbgHistEstoq do begin
      // monta as colunas abaixo se for verdadeiro
      if MontaColunaEstoqueProd Then begin

         DataSource := dmModule.dtsDataEntrEstoque;
        ...
        //Parametros da Coluna que exibe o Código do Produto
        Columns.Items[0].Title.Caption := 'Qtde.de Entrada';
        Columns.Items[0].FieldName     := 'QTDE';
        Columns.Items[0].Width         := 88;
        Columns.Items[0].Alignment     := taLeftJustify;

        //Parametros da Coluna que exibe o código de barras do produto
        Columns.Items[1].Title.Caption := 'Data';
        Columns.Items[1].FieldName     := 'DATA';
        Columns.Items[1].Width         := 97;
        Columns.Items[1].Alignment     := taLeftJustify;

        MontaColunaEstoqueProd := False;
...
////////////////////////////////////////////////////////////////////////////////
                                 // and(lbl_edtVlrCompraF1.Text > '0,00')
    if (cboFornecedores.Text <> '') Then begin
      ...
      if (cboFornecedores.ItemIndex <> - 1)Then begin
        ...
        ValorCustoForn1 := StringReplace(lbl_edtVlrCompraF1.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn1 := StringReplace(ValorCustoForn1,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores2.Text <> '') Then begin

      if (cboFornecedores2.ItemIndex <> - 1)Then begin

        IDFornProd2  := TClasseCombo( cboFornecedores2.Items.Objects[cboFornecedores2.ItemIndex] );

        ValorCustoForn2 := StringReplace(lbl_edtVlrCompraF2.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn2 := StringReplace(ValorCustoForn2,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores3.Text <> '') Then begin

      if (cboFornecedores3.ItemIndex <> - 1)Then begin

        IDFornProd3  := TClasseCombo( cboFornecedores3.Items.Objects[cboFornecedores3.ItemIndex] );

        ValorCustoForn3 := StringReplace(lbl_edtVlrCompraF3.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn3 := StringReplace(ValorCustoForn3,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores4.Text <> '') Then begin

      if (cboFornecedores4.ItemIndex <> - 1)Then begin

        IDFornProd4  := TClasseCombo( cboFornecedores4.Items.Objects[cboFornecedores4.ItemIndex] );

        ValorCustoForn4 := StringReplace(lbl_edtVlrCompraF4.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn4 := StringReplace(ValorCustoForn4,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores5.Text <> '') Then begin

      if (cboFornecedores5.ItemIndex <> - 1)Then begin

        IDFornProd5  := TClasseCombo( cboFornecedores5.Items.Objects[cboFornecedores5.ItemIndex] );

        ValorCustoForn5 := StringReplace(lbl_edtVlrCompraF5.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn5 := StringReplace(ValorCustoForn5,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores6.Text <> '') Then begin

      if (cboFornecedores6.ItemIndex <> - 1)Then begin

        IDFornProd6  := TClasseCombo( cboFornecedores6.Items.Objects[cboFornecedores6.ItemIndex] );

        ValorCustoForn6 := StringReplace(lbl_edtVlrCompraF6.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn6 := StringReplace(ValorCustoForn6,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores7.Text <> '') Then begin

      if (cboFornecedores7.ItemIndex <> - 1)Then begin

        IDFornProd7  := TClasseCombo( cboFornecedores7.Items.Objects[cboFornecedores7.ItemIndex] );

        ValorCustoForn7 := StringReplace(lbl_edtVlrCompraF7.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn7 := StringReplace(ValorCustoForn7,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (cboFornecedores8.Text <> '') Then begin

      if (cboFornecedores8.ItemIndex <> - 1)Then begin

        IDFornProd8  := TClasseCombo( cboFornecedores8.Items.Objects[cboFornecedores8.ItemIndex] );

        ValorCustoForn8 := StringReplace(lbl_edtVlrCompraF8.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
        ValorCustoForn8 := StringReplace(ValorCustoForn8,FormatSettings.DecimalSeparator,'.',[rfReplaceAll]);
...
////////////////////////////////////////////////////////////////////////////////
         ...
         { Commit(ibProdutos);
          ibProdutos.Close;
          ibProdutos.SQL.Clear;
          ibProdutos.SQL.Add('SELECT CADPRODUTOS.*,TBLENTRADAESTOQUEPROD.FORNECEDOR,'
          + 'TBLENTRADAESTOQUEPROD.ESTOQUEMIN,TBLENTRADAESTOQUEPROD.QUANTIDADE As QtdeEstoque FROM CADPRODUTOS '
          + 'INNER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD WHERE(TBLENTRADAESTOQUEPROD.FORNECEDOR IS NULL OR TBLENTRADAESTOQUEPROD.FORNECEDOR=''0'')');
          ibProdutos.Open; }                                                                                                                                                               // ORDER BY CADPRODUTOS.NOME
...
begin
  ...
  with dmModule do begin
    ...
    if ibUser.FieldByName('EXCLUSAODEPRODUTOS').AsString = 'Bloqueado' Then begin
      ...
      Application.MessageBox('Atenção: Para Exclusão do item contate o administrador?','Notificação: Exclusão de Registro', MB_OK + mb_DefButton1+MB_ICONWARNING);
    ...
    if lbl_edtCusto.Text <> '' Then begin
      ...
      if lbl_edtLucro.Text <> '' Then begin// se for em branco executa rotina abaixo.
        ...
        if (Pos(',',lbl_edtLucro.Text)>0) and (lbl_edtLucro.Text <> '') Then begin
          ...
          Valor1 := StrToFloat(StringReplace(lbl_edtLucro.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
          Valor2 := StrToFloat(StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
    ...
    Casas := ReadInteger('Formatacoes','FormatoMoedaCasas',2);//atribui as chaves do arquivo ini na variavel
      ...
      if Casas = 2 Then begin
       ...
       if (lbl_edtVendaa.Text <> '') Then begin
         ...
         lbl_edtVendaa.Text := StringReplace(lbl_edtVendaa.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
  ...
  if (lbl_edtCusto.Text > '0,00') and (lbl_edtVendaa.Text <> '') Then begin
    ...
    if lbl_edtCusto.Text <> '' Then begin// se for em branco executa rotina abaixo.
      ...
      if (Pos(',',lbl_edtCusto.Text)>0) and (lbl_edtCusto.Text <> '') Then begin
        ...
        Valor1 := StrToFloat(StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
        Valor2 := StrToFloat(StringReplace(lbl_edtVendaa.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
        Valor3 := Valor2 - Valor1;
        Valor4 := Valor3 * 100;
...
begin
  ...
  if (lbl_edtQtdeEstoque.Text <> '') Then begin// se for em branco executa rotina abaixo.
    ...
    if (Pos(',',lbl_edtQtdeEstoque.Text)>0) and (lbl_edtQtdeEstoque.Text <> '') Then begin
      ...
      Valor1 := StrToFloat(StringReplace(lbl_edtQtdeEstoque.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
...
begin
  ...
  {  ibProdutos.Close;
   ...
   { Commit(ibProdutos);
    ibProdutos.Close;
    ibProdutos.SQL.Clear;
    ibProdutos.SQL.Add('SELECT CADPRODUTOS.CODBARRAS,CADPRODUTOS.NOME,CADPRODUTOS.IDCFOP,CADPRODUTOS.IDUNIDVENDA,'
    + 'CADPRODUTOS.VALORCUSTO, CADPRODUTOS.VALORVENDAA,CADPRODUTOS.IDPROD,CADPRODUTOS.TIPOPROD,'
    + 'CADPRODUTOS.FORNECEDOR,CADPRODUTOS.VALORVENDAB,CADPRODUTOS.VALORVENDAC,CADPRODUTOS.IDCSOSN,'
    ...
    + 'CADPRODUTOS.QTDEEMBALAGEM,CADPRODUTOS.UNIDADE,CADPRODUTOS.UNCOMPRA,CADPRODUTOS.IDCST,CADPRODUTOS.IDST,'
    + 'CADPRODUTOS.QUANTIDADE,CADPRODUTOS.ESTGERAL,CADPRODUTOS.REF,'
    + 'CADPRODUTOS.QTDEPROM,CADPRODUTOS.VLRPROM,CADPRODUTOS.INDICACAO,CADPRODUTOS.FABRICANTE,'
    + 'CADPRODUTOS.FORNECEDOR2,CADPRODUTOS.FORNECEDOR3,CADPRODUTOS.FORNECEDOR4,CADPRODUTOS.FORNECEDOR5,'
    + 'CADPRODUTOS.FORNECEDOR6,CADPRODUTOS.FORNECEDOR7,CADPRODUTOS.FORNECEDOR8,'
    + 'CADPRODUTOS.COMISSAOPROD,CADPRODUTOS.COMISSAOPRECOC,CADPRODUTOS.LOCALIZACAOPROD,'
    + 'CADPRODUTOS.QTDEB,CADPRODUTOS.QTDEC,CADPRODUTOS.
    + 'TBLENTRADAESTOQUEPROD.ESTOQUEMIN,TBLENTRADAESTOQUEPROD.QUANTIDADE As QtdeEstoque,'
    + 'LOCALESTOCAGEM.NOME As Estoque FROM CADPRODUTOS '
    + 'INNER JOIN LOCALESTOCAGEM ON CADPRODUTOS.IDLOCAL=LOCALESTOCAGEM.IDESTOQUE '
    + 'LEFT OUTER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD ORDER BY CADPRODUTOS.NOME');
    ...
    + 'TBLENTRADAESTOQUEPROD.ESTOQUEMIN,TBLENTRADAESTOQUEPROD.QUANTIDADE As QtdeEstoque,'
    + 'LOCALESTOCAGEM.NOME As Estoque FROM CADPRODUTOS '
    + 'LEFT OUTER JOIN LOCALESTOCAGEM ON CADPRODUTOS.IDLOCAL=LOCALESTOCAGEM.IDESTOQUE '
    + 'LEFT OUTER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD ORDER BY CADPRODUTOS.NOME');
    ibProdutos.Open;
...
begin
  ...
  {  Commit(ibProdutos);
    ...
    Commit(ibDataEntrEstoque);
    ibDataEntrEstoque.Close;
    ibDataEntrEstoque.SQL.Clear;
    ibDataEntrEstoque.SQL.Add('SELECT QTDE,DATA,IDPROD FROM TBLDATAENTRESTOQUE WHERE IDPROD=''' + lbl_edtCodigo.Text + ''' ORDER BY TBLDATAENTRESTOQUE.DATA ASC');
    ibDataEntrEstoque.Open;
  ...
  with dmModule do begin
    ...
    if key=#13 Then begin
      ...
      if (lbl_edtResultado.Text > '0,00')and(lbl_edtVendaFormula.Text > '0,00') Then begin
        ...
        ConfirmaAlteracao := Application.MessageBox('Este procedimento altera o registro e acerta o estoque,clique em sim para continuar e nao pra cancelar?','Notificação: Exclusão de Registro', MB_YESNO + mb_Defbutton1+MB_ICONQUESTION);
...
begin
  ...
  if lbl_edtVendad.Text <> '' Then begin
    ...
    lbl_edtVendad.Text := StringReplace(lbl_edtVendad.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
      ...
      ibProdutoFornecedor.Open;
        ...
        TRY
         ...
         //  qrdbFornecedor.DataField     := ibProdutos.FieldByName('NomeFornecedor').FieldName;
            ...
            qrpProdutosFornecedor.Prepare;
           // ExportQR1.Report := frmRProdFornecedor.qrpProdutosFornecedor;
           // ExportQR1.ExportQRPDF('C:\Relatórios\''Estoque por Fornecedor Geral '+ cboFornecedoresProdutos.Text,false);
  ...
  with dmModule do begin
    ...
    case Key of
        ...
        VK_F2 :begin
          ...
          ibProdutos.SQL.Add('SELECT CADPRODUTOS.*,TBLENTRADAESTOQUEPROD.FORNECEDOR,'
          + 'TBLENTRADAESTOQUEPROD.ESTOQUEMIN,TBLENTRADAESTOQUEPROD.QUANTIDADE As QtdeEstoque FROM CADPRODUTOS '
          + 'INNER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD WHERE(TBLENTRADAESTOQUEPROD.FORNECEDOR IS NULL OR TBLENTRADAESTOQUEPROD.FORNECEDOR=''0'')ORDER BY CADPRODUTOS.NOME');
          ibProdutos.Open;
...
begin
  ...
  with dmModule do begin
    ...
    ConfirmaProd := Application.MessageBox('Confirma Inclusão?','Incluir Produto Equivalente',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
begin
  ...
  with dmModule do begin
    ...
    ConfirmaExclusao := Application.MessageBox('Confirma Exclusão?','Excluir Produto Equivalente',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
  ...
  if (lbl_edtCustoFormula.Text > '0,00') and (lbl_edtQtdeFormula.Text <> '') Then begin
    ...
    if lbl_edtCustoFormula.Text <> '' Then begin// se for em branco executa rotina abaixo.
      ...
      if (Pos(',',lbl_edtCustoFormula.Text)>0) and (lbl_edtCustoFormula.Text <> '') Then begin
        ...
        CustoNovo := StrToFloat(StringReplace(lbl_edtCustoFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
        QtdeNova := StrToFloat(StringReplace(lbl_edtQtdeFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
  ...
  if (lbl_edtCusto.Text > '0,00') and (lbl_edtEstoqgeral.Text <> '') Then begin
    ...
    if lbl_edtCusto.Text <> '' Then begin// se for em branco executa rotina abaixo.
      ...
      if (Pos(',',lbl_edtCusto.Text)>0) and (lbl_edtCusto.Text <> '') Then begin
        ...
        CustoAntigo := StrToFloat(StringReplace(lbl_edtCusto.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
        QtdeAntiga := StrToFloat(StringReplace(lbl_edtEstoqgeral.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
...
begin
       ...
       if (lbl_edtCustoFormula.Text <> '') Then begin
         ...
         lbl_edtCustoFormula.Text := StringReplace(lbl_edtCustoFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
...
begin
       ...
       if (lbl_edtQtdeFormula.Text <> '') Then begin
         ...
         lbl_edtQtdeFormula.Text := StringReplace(lbl_edtQtdeFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]);
    ...
    if lbl_edtCustoFormula.Text <> '' Then begin
      ...
      if lbl_edtLucro.Text <> '' Then begin// se for em branco executa rotina abaixo.
        ...
        if (Pos(',',lbl_edtLucro.Text)>0) and (lbl_edtLucro.Text <> '') Then begin
          ...
          Valor1 := StrToFloat(StringReplace(lbl_edtLucro.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
          Valor2 := StrToFloat(StringReplace(lbl_edtCustoFormula.Text,FormatSettings.ThousandSeparator,'',[rfReplaceAll]));
          Valor3 := Valor2 * Valor1;
...
procedure TfrmProd_MPrima.dbgProdutosColEnter(Sender: TObject);
begin

  with dmModule do begin

 // dbgProdutosCellClick(sender);
 //TrimAppMemorySize;
   { if dbgProdutos.SelectedIndex = 5 Then
      ...
      begin
       ...
       dbgProdutos.Options := dbgProdutos.Options - [dgEditing];
       showMessage('Campo não permitido pra alterar caralho');
...
procedure TfrmProd_MPrima.lbl_edtCodFabrEnter(Sender: TObject);
begin
tedit(sender).color := $00A0FAF8;
end;

procedure TfrmProd_MPrima.lbl_edtCodFabrExit(Sender: TObject);
begin
tedit(sender).color := clwindow;
end;

procedure TfrmProd_MPrima.lbl_edtCodFabrKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmProd_MPrima.lbl_edtComissaoProdKeyPress(Sender: TObject;
  var Key: Char);
begin

  if key=#13 Then begin//perform(wm_nextdlgctl,0,0);

    lbl_edtCodFabr.SetFocus;

  end;

end;

procedure TfrmProd_MPrima.lbl_edtComissaoProdEnter(Sender: TObject);
begin
tedit(sender).color := $00A0FAF8;
end;

procedure TfrmProd_MPrima.lbl_edtComissaoProdExit(Sender: TObject);
begin
tedit(sender).color := clwindow;
end;

procedure TfrmProd_MPrima.lbl_edtBuscaCodFabrChange(Sender: TObject);
begin
  ...
  ///  + 'TBLENTRADAESTOQUEPROD.ESTOQUEMIN,TBLENTRADAESTOQUEPROD.QUANTIDADE As QtdeEstoque,'
    + 'LOCALESTOCAGEM.NOME As Estoque FROM CADPRODUTOS '
    + 'INNER JOIN LOCALESTOCAGEM ON CADPRODUTOS.IDLOCAL=LOCALESTOCAGEM.IDESTOQUE WHERE CADPRODUTOS.CODIGOFABRICANTE=''' + lbl_edtBuscaCodFabr.Text + '''');
   /// + 'LEFT OUTER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD WHERE CADPRODUTOS.REF=''' + lbl_edtNumRef.Text + '''');
    ibProdutos.Open;
...
procedure TfrmProd_MPrima.lbl_edtBuscaCodFabrEnter(Sender: TObject);
begin
tedit(sender).color := $00A0FAF8;
end;

procedure TfrmProd_MPrima.lbl_edtBuscaCodFabrExit(Sender: TObject);
begin
tedit(sender).color := clwindow;
end;

procedure TfrmProd_MPrima.lbl_edtQtdePacoteFardoExit(Sender: TObject);
begin
tedit(sender).color := clwindow;
end;

procedure TfrmProd_MPrima.lbl_edtComissaoCEnter(Sender: TObject);
begin
tedit(sender).color := $00A0FAF8;
end;

procedure TfrmProd_MPrima.lbl_edtComissaoCExit(Sender: TObject);
begin
tedit(sender).color := clwindow;
end;

procedure TfrmProd_MPrima.lbl_edtComissaoCKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TfrmProd_MPrima.ckAcrescentarEstoqClick(Sender: TObject);
begin
lbl_edtQtdeEstoque.SetFocus;
end;

procedure TfrmProd_MPrima.ckDiminuirEstoqueClick(Sender: TObject);
begin
lbl_edtQtdeEstoque.SetFocus;
end;
