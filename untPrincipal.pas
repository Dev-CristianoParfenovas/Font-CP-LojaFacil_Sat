...
type
  TfrmPrincipal = class(TForm)
    ...
    procedure sbVendasMouseEnter(Sender: TObject);
    procedure sbVendasMouseLeave(Sender: TObject);
  ...
  with ConfigSiS do begin
    ...
    ConfigSiS := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'Data\RunTime\sis.ini'); //Cria o vinculo com o Arquivo ini
      with dmModule do begin
        ...
        dmModule.IBDGERAL.DatabaseName := ReadString('ConfigSiS','Banco',''); //Faz a leitura de uma String na seção do arquivo ini
...
procedure TfrmPrincipal.sbVendasMouseEnter(Sender: TObject);
begin
sbVendas.Font.Color := clBlack;
end;

procedure TfrmPrincipal.sbVendasMouseLeave(Sender: TObject);
begin
sbVendas.Font.Color := clWhite;
end;
...
begin
  ...
  with dmModule do begin
    ...
    ConfirmaProcedimento := Application.MessageBox('Atenção esse procedimento zera todo o Estoque?','ZERAR ESTOQUE GERAL',+MB_YESNO+MB_DEFBUTTON1+MB_ICONWARNING);
...
///  SkinData1.Active := False;
  ...
  ConfirmaMSG := Application.MessageBox('Não esqueça de fazer o backup?','Sair do Sistema', MB_YESNO + mb_DefButton1+MB_ICONQUESTION);
