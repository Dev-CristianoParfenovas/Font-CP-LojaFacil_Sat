...
uses
  ...
  Dialogs, StdCtrls, BmsXPCheckBox, BmsXPButton, ComCtrls,
  Grids, DBGrids, classComboBox, DB, IBQuery, ExtCtrls;

type
  TfrmContasaPagar = class(TForm)
    ckFornecedor: TBmsXPCheckBox;
    ckServicos: TBmsXPCheckBox;
    ibeaIncluirCP: TBmsXPButton;
...
////////////////////////////////////////////////////////////////////////////////
      ...
      while not ibCPagar.Eof do begin
        ...
        if TotalTbl = 0 Then begin
          ...
          TotalTbl := StrToFloat(StringReplace(FloatToStr(ibCPagar.FieldByName('VLREMITIDO').AsFloat),ThousandSeparator,'',[rfreplaceAll]));

        end else begin

          TotalCP := StringReplace(FloatToStr(ibCPagar.FieldByName('VLREMITIDO').AsFloat),ThousandSeparator,'',[rfReplaceAll]);

          TotalTbl := TotalTbl + StrtoFloat(StringReplace(TotalCP,ThousandSeparator,'',[rfReplaceAll]));
...
begin
  ...
  with dmModule do begin
   ...
   { if ckCustoDiario.Checked Then begin
    ...
    ValorEmitido := StringReplace(lbl_edtVlrEmitidoCli.Text,ThousandSeparator,'',[rfReplaceAll]);
  ...
  with dmModule do begin
    ...
    if (ckFornecedor.checked) Then begin
      ...
      if cboFornecedorServicos.ItemIndex = -1 Then begin
        ...
        //Application.MessageBox('Informe o Tanque','Notificação: Tanque não especificado',MB_OK+MB_ICONERROR);
  ...
  with dmModule do begin
    ...
    with dbgContasaPagar do begin
      ...
      if MontaColunas Then begin
        ...
        Columns.Items[0].Title.Caption := 'Serviços';
        ...
        // Parametros que exibe o Bico da Bomba
        Columns.Items[3].Title.Caption := 'NºDoc.';
...
begin
  ...
  ConfirmaGravacao := Application.MessageBox('Confirma o Registro?','Inclusão de registro',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
begin
  ...
  ConfirmaExclusao := Application.MessageBox('Atenção este procedimento baixa o contas a pagar do período determinado, deseja continuar??','Baixa do Contas a pagar',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
begin
  ...
  ConfirmaExclusao := Application.MessageBox('Atenção este procedimento retorna o contas a pagar do período determinado, deseja continuar??','Retorno do Contas a Pagar',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
begin
  ...
  ConfirmaExclusao := Application.MessageBox('Atenção este procedimento baixa o contas a pagar selecionado, deseja continuar??','Baixa do Contas a pagar',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
begin
  ...
  ConfirmaExclusao := Application.MessageBox('Atenção este procedimento retorna o contas a pagar selecionado, deseja continuar??','Retorno do Contas a pagar',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
    ...
    if lbl_edtVlrAmor.Text <> '' Then begin
      ...
      if (IDContasaPagar > 0)and(lbl_edtVlrEmitidoCli.Text > '0,00')Then begin
        ...
        if ConfirmaAbat = 6 Then begin
          ...
          ValorDocumento := StrToFloat(StringReplace(lbl_edtVlrEmitidoCli.Text,ThousandSeparator,'',[rfReplaceAll]));
          ValorAbater    := StrToFloat(StringReplace(lbl_edtVlrAmor.Text,ThousandSeparator,'',[rfReplaceAll]));
          ...
          ValorAmortizado := StrToFloat(StringReplace(FloatToStr(ibCPagar.FieldByName('VALORABATER').AsFloat),ThousandSeparator,'',[rfreplaceAll]));

          TotalDocumento := StringReplace(FloatToStr(Valor),ThousandSeparator,'',[rfReplaceAll]);
          TotalDocumento := StringReplace(TotalDocumento,DecimalSeparator,'.',[rfReplaceAll]);

          if ValorAbater > 0 Then begin

          VlrAmor        := StringReplace(FloatToStr(ValorAbater + ValorAmortizado),ThousandSeparator,'',[rfReplaceAll]);
          VlrAmor := StringReplace(VlrAmor,DecimalSeparator,'.',[rfReplaceAll]);
