...
type
  TfrmBuscaProdutos = class(TForm)
    ...
    lbl_edtCodBarrasAd: TmxFlatEdit;
    lbl_CodBarras: TLabel;
    lbl_edtCodInterno: TmxFlatEdit;
    lbl_CodRef: TLabel;
    lbl_edtNome: TmxFlatEdit;
    lbl_NomeBusca: TLabel;
    lbl_edtProduto: TmxFlatEdit;
    lbl_ProdutoDescr: TLabel;
    lbl_edtQtdeEstoque: TmxFlatEdit;
    lbl_QtdeEStoq: TLabel;
    lbl_edtPrecoVenda: TmxFlatEdit;
    lbl_PrecoA: TLabel;
    lbl_edtFabricanteProd: TmxFlatEdit;
    lbl_Fabr: TLabel;
    Image2: TImage;
    lbl_Info: TLabel;
    cboFornecedores: TmxFlatComboBox;
    lbl_Forn: TLabel;
    lbl_edtIDProd: TmxFlatEdit;
    ckPrecob: TBmsXPCheckBox;
    ckPrecoc: TBmsXPCheckBox;
    ckPrecoD: TBmsXPCheckBox;
    lbl_F3: TLabel;
    lbl_edtValor: TmxFlatEdit;
    lbl_Vlr: TLabel;
    lbl_F2: TLabel;
    lbl_F4: TLabel;
    ckBuscaLetra: TBmsXPCheckBox;
    ckBuscaNome: TBmsXPCheckBox;
    dbgProdEquivalente: TDBGrid;
    lbl_ProdEquivalente: TLabel;
    cboEstoque: TmxFlatComboBox;
    lbl_Estoque: TLabel;
    lbl_edtIDEstoque: TLabeledEdit;
    ...
    procedure dbgProdEquivalenteCellClick(Column: TColumn);
    procedure cboEstoqueChange(Sender: TObject);
  private
...
var
  ...
  MontaColunas,MontaColEquivalente : Boolean;
  IDProd : Integer;
  ...
  RefProd : String;
  IDProdConsulta,IDProdEqConsulta : Integer;
  
implementation

uses untdmModule, funcPosto, untServicos,
  untCaixa, untPedidoVenda, untCliente, untPosVenda, untTroca,
  untNotaFiscal, untPedidodeCompra, untCotacao, untEtiquetasNomePrat,
  untEntradadeEstoque, untProdutos_MPrima;
...
procedure TfrmBuscaProdutos.ExecSelectEstoque;

var

IDClassEstoque : TClasseCombo;
begin
  ...
  with dmModule do begin
    ...
    ibEstoque.Close;
    ibEstoque.SQL.Clear;
    ibEstoque.SQL.Add('SELECT * FROM LOCALESTOCAGEM');//('SELECT * FROM TBLCADASTRO WHERE IDCLASS=''' + InttoStr(IDClass) + ''' ORDER BY NOME');
    ibEstoque.Open;

    cboEstoque.Clear; //LIMPA O COMBOBOX
    while not ibEstoque.Eof do begin
      IDClassEstoque     := TClasseCombo.Create;
      IDClassEstoque.ID  := ibEstoque.FieldByName('IDESTOQUE').AsInteger;
      cboEstoque.Items.AddObject(ibEstoque.FieldByName('NOME').AsString,IDClassEstoque);
      ibEstoque.Next;
    end;
  end;
...
begin
  ...
  {  Commit(ibTempProdNome);
    ...
    ibProdutosEquivalentes.SQL.Add('SELECT * FROM CADPRODUTOS ORDER BY CADPRODUTOS.NOME');//.*,TBLCADASTRO.NOME FROM CADPRODUTOS '
...
////////////////////////////////////////////////////////////////////////////////
    ...
    with dbgProdEquivalente do begin

      if MontaColEquivalente Then begin

      DataSource := dtsProdEquivalente;
...
begin
 ...
 //     RefConsulta := lbl_edtCodInterno.Text;
      ...
      ibProdutosEquivalentes.Close;
      ibProdutosEquivalentes.SQL.Clear;
      ibProdutosEquivalentes.SQL.Add('SELECT * FROM CADPRODUTOS WHERE CADPRODUTOS.IDPROD=''' + IntToStr(IDProdConsulta) + '''');
    ...
    //  + 'LEFT OUTER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD WHERE CADPRODUTOS.IDPROD=''' + IntToStr(IDProdConsulta) + '''');
      ...
      if ibProdutosEquivalentes.RecordCount > 0 Then begin
        ...
        lbl_edtProduto.Text    := ibProdutosEquivalentes.FieldByName('NOME').AsString;
       ...
       // lbl_edtQtdeEstoque.Text := (FormatFloat(CasasDecimais('Produtos'),StrToFloat(ibTempProdNome.FieldByName('TotalEstoque').AsString)));
        ...
        lbl_edtCodBarrasAd.Text   := ibProdutosEquivalentes.FieldByName('CODBARRAS').AsString;
          ...
          ibProdEquivalente.SQL.Add('SELECT * FROM TBLPRODEQUIVALENTE WHERE IDPRODDETALHEE=''' + IntToStr(IDProdConsulta) + '''');
...
begin
  ...
  with dmModule do begin
   ...
   ///    ShowMessage('FAROFA,FABIO,FACULDADE'); or(frmVendasLoja.Showing)
      ...
      /// frmVendasLoja.lbl_IDEstoqueProd.Caption := lbl_edtIDEstoque.Text;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    { (ibTempProd.FieldByName('VALORCUSTO') as TFloatField).DisplayFormat := ',0.00';
     ...
     (ibTempProd.FieldByName('VALORVENDAD') as TFloatField).DisplayFormat := ',0.00'; }
      ...
      lbl_QtdeEstoque.Caption := 'Qtde. Estoque: '+(FormatFloat(CasasDecimais('Produtos'),ibEntrProdEstoque.FieldByName('QUANTIDADE').AsFloat));//(FormatFloat('0',FLOATTO(ibEntrProdEstoque.FieldByName('QUANTIDADE').AsString)));
...
begin
  ...
  with dmModule do begin
    ...
    if lbl_edtCodBarrasAd.Text <> '' Then begin
      ...
      ibProdutosEquivalentes.Close;
      ibProdutosEquivalentes.SQL.Clear;
      ibProdutosEquivalentes.SQL.Add('SELECT * FROM CADPRODUTOS WHERE CADPRODUTOS.CODBARRAS=''' + CodBarraConsulta + '''');//.*,TBLCADASTRO.NOME FROM CADPRODUTOS '
...
begin
  ...
  with dmModule do begin
    ...
    ibProdutosEquivalentes.Close;
    ibProdutosEquivalentes.SQL.Clear;
    ibProdutosEquivalentes.SQL.Add('SELECT * FROM CADPRODUTOS WHERE CADPRODUTOS.NOME LIKE '+''''+lbl_edtNome.text+'%'+''' ORDER BY NOME');
...
begin
  ...
  with dmModule do begin
    ...
    if lbl_edtCodInterno.Text <> '' Then begin
      ...
      ibProdutosEquivalentes.Close;
      ibProdutosEquivalentes.SQL.Clear;
      ibProdutosEquivalentes.SQL.Add('SELECT * FROM CADPRODUTOS WHERE CADPRODUTOS.REF=''' + Ref + '''');//.*,TBLCADASTRO.NOME FROM CADPRODUTOS '
...
begin
  ...
  with dmModule do begin
    ...
    if lbl_edtValor.Text <> '' Then begin
      ...
      ibProdutosEquivalentes.Close;
      ibProdutosEquivalentes.SQL.Clear;
      ibProdutosEquivalentes.SQL.Add('SELECT * FROM CADPRODUTOS WHERE CADPRODUTOS.VALORVENDAA ='''+ ValorProduto +'''');
      ibProdutosEquivalentes.Open;
      (ibProdutosEquivalentes.FieldByName('VALORVENDAA') as TFloatField).DisplayFormat := CasasDecimais('Produtos'); //CRIA UMA MASCARA PARA O CAMPO
...
procedure TfrmBuscaProdutos.dbgProdEquivalenteCellClick(Column: TColumn);
begin

  with dmModule do begin

    IDProdEqConsulta := ibProdEquivalente.FieldByName('IDPROD').AsInteger;

      Commit(ibProdutos);
      ibProdutos.Close;
      ibProdutos.SQL.Clear;
      ibProdutos.SQL.Add('SELECT * FROM CADPRODUTOS WHERE CADPRODUTOS.IDPROD=''' + IntToStr(IDProdEqConsulta) + '''');
    //  + 'TBLENTRADAESTOQUEPROD.QUANTIDADE As TotalEstoque FROM CADPRODUTOS '//TBLCADASTRO.NOME FROM CADPRODUTOS '
    //  + 'LEFT OUTER JOIN TBLCADASTRO ON CADPRODUTOS.FORNECEDOR=TBLCADASTRO.IDCAD '
    //  + 'LEFT OUTER JOIN TBLENTRADAESTOQUEPROD ON CADPRODUTOS.IDPROD=TBLENTRADAESTOQUEPROD.IDPROD WHERE CADPRODUTOS.IDPROD=''' + IntToStr(IDProdConsulta) + '''');
      ibProdutos.Open;
      ...
      if ibProdutos.RecordCount > 0 Then begin
        ...
        lbl_edtProduto.Text    := ibProdutos.FieldByName('NOME').AsString;
        lbl_edtPrecoVenda.Text := FormatFloat(CasasDecimais('Produtos'),ibProdutos.FieldByName('VALORVENDAA').AsFloat);{ibTempProd.FieldByName('VALORVENDAA').AsString;}//(ibProdutos.FieldByName('VALORVENDAA') as TFloatField).DisplayFormat := CasasDecimais('Produtos'); //CRIA UMA MASCARA PARA O CAMPO
       // lbl_edtQtdeEstoque.Text := (FormatFloat(CasasDecimais('Produtos'),StrToFloat(ibTempProdNome.FieldByName('TotalEstoque').AsString)));
        lbl_edtFabricanteProd.Text := ibProdutos.FieldByName('FABRICANTE').AsString;
        lbl_edtCodInterno.Text := ibProdutos.FieldByName('REF').AsString;   //:= IntToStr(ibTempProdNome.FieldByName('IDPROD').AsInteger);
        lbl_edtIDProd.Text := IntToStr(IDProdEqConsulta);
        lbl_edtCodBarrasAd.Text   := ibProdutos.FieldByName('CODBARRAS').AsString;
      ...
      ibEntrProdEstoque.Close;
      ibEntrProdEstoque.SQL.Clear;
      ibEntrProdEstoque.SQL.Add('SELECT IDPROD,QUANTIDADE FROM TBLENTRADAESTOQUEPROD WHERE IDPROD=''' + IntToStr(IDProdEqConsulta) + '''');
      ibEntrProdEstoque.Open;
...
procedure TfrmBuscaProdutos.cboEstoqueChange(Sender: TObject);
...
begin
  ...
  with dmModule do begin
    ...
    if cboEstoque.ItemIndex <> -1 Then begin//SE HOUVER REGISTRO NO COMBO
      IDEstoqueCons         := TClasseCombo( cboEstoque.Items.Objects[cboEstoque.ItemIndex] );//ATRIBUI A CLASSE COMBO NA VARIAVEL
      ...
      + 'WHERE TBLENTRADAESTOQUEPROD.IDLOCAL=''' + IntToStr(IDEstoqueCons.ID) + ''' and TBLENTRADAESTOQUEPROD.IDPROD=''' + lbl_edtIDProd.Text + '''');
      ibEntrProdEstoque.Open;

      (ibEntrProdEstoque.FieldByName('VLRTOTAL') as TFloatField).DisplayFormat := ',0.00';

      lbl_edtQtdeEstoque.Text := FormatFloat(CasasDecimais('Produtos'),ibEntrProdEstoque.FieldByName('QUANTIDADE').AsFloat);
      lbl_edtIDEstoque.Text   := IntToStr(IDEstoqueCons.ID);
