       ...
       //  TotaldoMinimo := StrToInt(QtdeMinimaEstoque);
          ...
          if InfoEstoqueMinimo = 'Sim' Then begin
            ...
            if (MinimoTabela = QtdeEstoqueVenda)or(MinimoTabela > QtdeEstoqueVenda) Then begin
              ...
              Application.MessageBox(PChar('Atenção a Prateleira de:"'+ ibEntrProdEstoque.FieldByName('NOME').AsString +'", está em sua Quantidade Mínima!!!'),'Notificação: Prateleira na qtde mínima.',+MB_OK+MB_ICONWARNING);
       ...
       //  TotaldoMinimo := StrToInt(QtdeMinimaEstoque);
          ...
          if InfoEstoqueMinimo = 'Sim' Then begin
            ...
            if (MinimoTabela = QtdeEstoqueVenda)or(MinimoTabela > QtdeEstoqueVenda) Then begin
              ...
              Application.MessageBox(PChar('Atenção o Estoque de:"'+ ibEntrProdEstoque.FieldByName('NOME').AsString +'", está em sua Quantidade Mínima!!!'),'Notificação: Estoque na qtde mínima.',+MB_OK+MB_ICONWARNING);
...
//    EpsonMFD           := ibConfig.FieldByName('EPSONMFD').AsString;
  ...
  { with ConfigSiSCX do begin
      ...
      with dmModule do begin
        ...
        dmModule.IBDCX.DatabaseName := ReadString('ConfigSiSCX','Banco','') + '\LOJACX.fdb'; //Faz a leitura de uma String na seção do arquivo ini
    ...
    {  if ImpressaoTermica = 'Sim' then begin
      ...
      VK_f4:begin
        ...
        if (lbl_edtCodBarras.Enabled = True)and(lbl_edtCodProd.Enabled = True) Then begin
          ...
          AvisodeCupom := Application.MessageBox('É necessário terminar esta venda para um novo cupom de venda?','**Venda não finalizada**',+MB_OK+MB_DEFBUTTON1+MB_ICONINFORMATION);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    //  lbl_edtQtde.SetFocus;
      ...
      lbl_edtQtde.Text := '1';  
       ...
       { ConfirmaConfiguracao := Application.MessageBox('Confirma Configuração da Impressora?','Config.Impressora',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
//  try
    ...
    if (Length(TextoCodigo) = 13) and (TextoCodigo[1] = '2') then begin//Verifica se é produto de pesagem
      ...
      if (NomeUnidadeVenda = 'KG/QTDE')or(NomeUnidadeVenda = 'KG/VLR')Then begin
          ...
          if ibProdutos.RecordCount > 0 Then begin
            ...
            if NomeUnidadeVenda = 'KG/QTDE' Then begin
              ...
              lbl_edtCodBarras.Text := Copy(TextoCodigo,2,6);//Coloco o código do produto no Edit de código do produto
         ...
         {  intQtde := StrToInt(lbl_edtQtde.Text);
           ...
           if intQtde > 100 Then begin
             ...
             ShowMessage('Atenção a qtde. está maior que 100?');
         ...
         if lbl_edtIDProdCX.Text > '0' Then begin
           ...
           if intQtde > 100 Then begin
             ...
             if (intVendaEstoqueNulo = 1)and(intQtde <= 0) Then begin
               ...
               ConfirmaVendaEstoque := Application.MessageBox('Produto indisponível no estoque,'
               + #13 + 'clique em sim para continuar e não para verificação','Notificação: Produto sem estoque',MB_YESNO + MB_ICONQUESTION);
...
{ with dmModule do begin
    ...
    if (int_Confirma = 6) then begin  //Int_Confirma = 6(Sim), = 7(Não), é o valor de retorno do Application.MessageBox
  ...
  with dmModule do begin
    ...
    if key=#13 Then begin
      ...
      lbl_edtQtde.SetFocus;
        ...
        ConfirmaCancelamentoVenda := Application.MessageBox('Confirma o cancelamento','exclusão de registro',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
...
////////////////////////////////////////////////////////////////////////////////
         ...
         { Commit(ibConfig);
          ...
          if AvisaCupom = 'Sim' Then begin
           ...
           Int_Retorno:= Daruma_DUAL_ImprimirTexto(pchar('*--------------------------------------*'),0);
          ///Int_Retorno:= Daruma_DUAL_ImprimirTexto(pchar('Cupom Nº.:<sp>1</sp>'+frmVendasLoja.lbl_CodVenda.Caption+ ''),0);
...
//VARIÁVEIS PARA CANCELAR ÚLTIMA VENDA DO SISTEMA
  ...
  with dmModule do begin
   ...
   // BematechMFD        := ibConfig.FieldByName('BEMATECHMFD').AsString;
    ...
    ConfirmaCancelamento := Application.MessageBox('Confirma o Cancelamento do Cupom?','Cancelamento de Cupom',+MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
      ...
      ConfirmaUltimoCupom := Application.MessageBox('Este procedimento também cancela o último cupom do Sistema,Clique em sim para continuar ou não para cancelar somente da impressora','Cancelamento',+MB_YESNO+MB_DEFBUTTON1+MB_ICONINFORMATION);
...
////////////////////////////////////////////////////////////////////////////////
      ...
      Application.MessageBox('Você optou por não cancelar o último cupom do sistema,lembre-se que só foi cancelado a última venda na impressora,'+
      'no sistema cancele o último cupom no módulo manutenção de cupons',+MB_OK);
...
begin  
  ...
  with dmModule do begin
    ...
    if (lbl_edtCodBarras.Enabled = False)and(lbl_edtCodProd.Enabled = False) Then begin
      ...
      frmVendasLoja.lbl_Orcamento.Caption := 'Orçamento Nº:';
...
begin
  ...
  with dmModule do begin
    ...
    intEncerrarOrc := Application.MessageBox('Deseja encerrar o orçamento?','Orçamento', +MB_YESNO+MB_DEFBUTTON1+MB_ICONQUESTION);
        ...
        TRY
          ...
          with frmR_Orc do begin
             ...
             // qrdbData.DataField  := ibServsVenda.FieldByName('DATA').FieldName;
              ...
              //ExportQR1.ExportQRPDF('C:\Orçamentos\''Orçamento Nº '+strOrc+'  '+ strDataOrc+'',false);
             // ExportQR1.ExportQR1('C:\Pedidos_Email\''Nº '+strNpedido+'  '+ ClientePedido+'',false);
