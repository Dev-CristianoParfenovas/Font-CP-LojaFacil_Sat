object frmPedidosLoja: TfrmPedidosLoja
  Left = 181
  Top = 7
  BorderIcons = []
  Caption = 'Pedidos Loja'
  ClientHeight = 471
  ClientWidth = 775
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lbl_F2: TLabel
    Left = 724
    Top = 42
    Width = 49
    Height = 13
    Caption = 'F2 - Sair'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lbl_F3: TLabel
    Left = 8
    Top = 443
    Width = 232
    Height = 13
    Caption = 'F3 - Para selecionar um Cliente na grade'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lbl_F4: TLabel
    Left = 8
    Top = 461
    Width = 186
    Height = 13
    Caption = 'F4 - Permite Cancelar um Pedido'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object ibeaIncluir: TSpeedButton
    Left = 401
    Top = 67
    Width = 66
    Height = 34
    Caption = '&Novo'
    Flat = True
    Glyph.Data = {
      
      0000333903333333333330000000333033333333333330000000}
    OnClick = ibeaIncluirClick
  end
  object ibeaSair: TSpeedButton
    Left = 715
    Top = 5
    Width = 63
    Height = 34
    Caption = '&Sair'
    Flat = True
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    OnClick = ibeaSairClick
  end
  object lbl_DataIni: TLabel
    Left = 9
    Top = 8
    Width = 70
    Height = 13
    Caption = 'Data Inicial:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lbl_DataF: TLabel
    Left = 121
    Top = 8
    Width = 63
    Height = 13
    Caption = 'Data Final:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lbl_IDFunc: TLabel
    Left = 555
    Top = 96
    Width = 21
    Height = 13
    AutoSize = False
    Caption = '.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object dbgPedidoLoja: TDBGrid
    Left = 7
    Top = 119
    Width = 766
    Height = 315
    Cursor = crHandPoint
    BorderStyle = bsNone
    FixedColor = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Verdana'
    Font.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnKeyDown = dbgPedidoLojaKeyDown
    OnKeyPress = dbgPedidoLojaKeyPress
    OnKeyUp = dbgPedidoLojaKeyUp
  end
  object lbl_edtNomerazao: TLabeledEdit
    Left = 118
    Top = 76
    Width = 279
    Height = 19
    BevelInner = bvSpace
    BevelKind = bkFlat
    BorderStyle = bsNone
    EditLabel.Width = 40
    EditLabel.Height = 13
    EditLabel.Caption = 'Cliente'
    EditLabel.Font.Charset = DEFAULT_CHARSET
    EditLabel.Font.Color = clWindowText
    EditLabel.Font.Height = -11
    EditLabel.Font.Name = 'MS Sans Serif'
    EditLabel.Font.Style = [fsBold]
    EditLabel.ParentFont = False
    TabOrder = 1
    OnExit = lbl_edtNomerazaoExit
    OnKeyPress = lbl_edtNomerazaoKeyPress
  end
  object lbl_edtNumPedido: TLabeledEdit
    Left = 9
    Top = 76
    Width = 103
    Height = 19
    BevelInner = bvSpace
    BevelKind = bkFlat
    BorderStyle = bsNone
    EditLabel.Width = 80
    EditLabel.Height = 13
    EditLabel.Caption = 'N'#186' do Pedido:'
    EditLabel.Font.Charset = DEFAULT_CHARSET
    EditLabel.Font.Color = clWindowText
    EditLabel.Font.Height = -11
    EditLabel.Font.Name = 'MS Sans Serif'
    EditLabel.Font.Style = [fsBold]
    EditLabel.ParentFont = False
    TabOrder = 0
    OnExit = lbl_edtNumPedidoExit
    OnKeyPress = lbl_edtNumPedidoKeyPress
  end
  object lbl_edtCodCli: TLabeledEdit
    Left = 553
    Top = 76
    Width = 103
    Height = 19
    BevelInner = bvSpace
    BevelKind = bkFlat
    BorderStyle = bsNone
    EditLabel.Width = 66
    EditLabel.Height = 13
    EditLabel.Caption = 'C'#243'd.Cliente'
    EditLabel.Font.Charset = DEFAULT_CHARSET
    EditLabel.Font.Color = clWindowText
    EditLabel.Font.Height = -11
    EditLabel.Font.Name = 'MS Sans Serif'
    EditLabel.Font.Style = [fsBold]
    EditLabel.ParentFont = False
    ReadOnly = True
    TabOrder = 3
    Visible = False
  end
  object lbl_edtIDVenda: TLabeledEdit
    Left = 476
    Top = 76
    Width = 64
    Height = 19
    BevelInner = bvSpace
    BevelKind = bkFlat
    BorderStyle = bsNone
    EditLabel.Width = 63
    EditLabel.Height = 13
    EditLabel.Caption = 'C'#243'd.Venda'
    EditLabel.Font.Charset = DEFAULT_CHARSET
    EditLabel.Font.Color = clWindowText
    EditLabel.Font.Height = -11
    EditLabel.Font.Name = 'MS Sans Serif'
    EditLabel.Font.Style = [fsBold]
    EditLabel.ParentFont = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    ReadOnly = True
    TabOrder = 4
    OnKeyPress = lbl_edtIDVendaKeyPress
  end
  object dtpDataIni: TDateTimePicker
    Left = 9
    Top = 24
    Width = 94
    Height = 21
    BevelInner = bvSpace
    BevelOuter = bvSpace
    BevelKind = bkFlat
    Date = 43207.000000000000000000
    Time = 0.866485277780157000
    TabOrder = 5
  end
  object dtpDataF: TDateTimePicker
    Left = 121
    Top = 24
    Width = 94
    Height = 21
    BevelInner = bvSpace
    BevelOuter = bvSpace
    BevelKind = bkFlat
    Date = 43207.000000000000000000
    Time = 0.866485277780157000
    TabOrder = 6
    OnChange = dtpDataFChange
  end
end
